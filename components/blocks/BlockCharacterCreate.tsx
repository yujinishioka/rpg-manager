...
import { AbilityScores, Detail, SkillsProficiency } from "@/types/index";
...
export default function BlockCharacterCreate({ data }: any) {
  ...
  const skills_proficiency = [
    ...
    { name: "Animal Handling", value: "sleight_of_hand", ability: "wis" },
    ...
    { name: "Sleight of Hand", value: "sleight_of_hand", ability: "dex" },
  ...
  const handleSubRaceChange = (value: string) => {
    const subrace = selectedRace.subrace.division.find(
      (subrace: any) => subrace.name === value
    );
  ...
  const handleClassChange = (value: string) => {
    const selectedClass = classes.find(
      (classType: any) => classType.name === value
    );
  ...
  const handleCharacter = () => {
    ...
    selectedRace
      ? console.log(`race: ${selectedRace.name}`)
    ...
    selectedMorality && selectedAttitude
      ? console.log(`alignment: ${selectedMorality} ${selectedAttitude}`)
      : console.log("alignment incomplete");
  };
  ...
  const subraceEmpty = () =>
    selectedRace !== null &&
    selectedRace.subrace !== null &&
    selectedSubRace === null;
  const classEmpty = () => selectedClass === null;
  const alignmentEmpty = () =>
    selectedMorality === null || selectedAttitude === null;
  const fieldsEmpty = () =>
    nameEmpty() ||
    raceEmpty() ||
    subraceEmpty() ||
        ...
        <div className="flex flex-col gap-2">
          ...
          {selectedRace && selectedRace.subrace !== null && (
            <label className="flex gap-2 items-center justify-between">
              ...
              >
                ...
                <SelectContent>
                  {selectedRace.subrace.division?.length > 0 &&
                    selectedRace.subrace.division.map((subrace: any) => (
                      <SelectItem
                        key={`race-${subrace.id}`}
                        value={subrace.name}
                      >
          ...
          <label className="flex gap-2 items-center justify-between">
            ...
            >
              ...
              <SelectContent>
                {classes?.length > 0 &&
                  classes.map((classRPG: any) => (
                    <SelectItem
                      key={`class-${classRPG.id}`}
                      value={classRPG.name}
                    >
                      {classRPG.name}
                    </SelectItem>
          ...
          <div className="items-center justify-between sm:flex sm:gap-2">
            ...
            <div className="flex gap-2">
              ...
              >
                ...
                <SelectContent>
                  {morality?.length > 0 &&
                    morality.map((item: any) => (
                      <SelectItem
                        key={`morality-${item.toLowerCase()}`}
              ...
              >
                ...
                <SelectContent>
                  {attitude?.length > 0 &&
                    attitude.map((item: any, index: number) => (
                      <SelectItem
                        key={`attitude-${index}`}
                        value={item.toLowerCase()}
                      >
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent className="bg-primary text-white">
                      <p className="max-w-[160px] text-center">
                        Roll 4 d6 and record the total of the highest three
                        dice.
                      </p>
                    </TooltipContent>
                  </Tooltip>
                  ...
                  >
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        checked={
                          skillsProficiency[
                            skill.name as keyof SkillsProficiency
                          ]
                        }
                        onCheckedChange={(e) =>
                          handleSkillsProficiencyChange(e, skill.value)
                        }
                      />
                      <label
                        htmlFor={skill.name}
                    ...
                    <p>
                      {abilityScores[skill.ability as keyof AbilityScores] +
                        (skillsProficiency[
                          skill.value as keyof SkillsProficiency
                        ]
                          ? 2
                          : 0)}
                    </p>
          ...
          <p className="my-3 font-semibold underline text-sm">
            It`s no longer obligatory from here on
          </p>
        ...
        <div className="flex justify-center pt-8 pb-6 gap-6 lg:pb-2">
          <div className="relative">
            ...
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger
                  className={`absolute -top-7 right-0 px-2 bg-primary/50 text-white z-50 rounded-lg hover:bg-primary/75`}
                >
                  {fieldsEmpty() ? "?" : ""}
                </TooltipTrigger>
          ...
          <Button onClick={handleClear} className="px-10">
            Reset
          </Button>
        </div>
      </div>
      <div className="flex flex-col gap-4 text-sm text-justify lg:w-[calc(100%-688px)]">
          ...
          <>
            {selectedRace.details?.length > 0 &&
              selectedRace.details.map((detail: any, index: number) => (
                <div
                  key={`race-details-${index}`}
                  className="flex flex-col gap-2 last:mb-0"
                >
            ...
            {selectedRace?.names && (
              <div className="flex flex-col gap-2">
                ...
                <div className="flex flex-col gap-1 text-sm">
                  {selectedRace.names?.types &&
                    namesExamplesByRace.map((type: any) => (
                      <div
                        key={`names-${selectedRace}-${type}`}
                        className="flex flex-wrap gap-1"
                      >
                        {selectedRace.names?.types[type] && (
                          <>
                            <h4 className="font-semibold">{type}:</h4>
                            {selectedRace.names.types[type].map(
                              (name: string) => (
        ...
        {selectedSubRace && (
          <>
            ...
            <Html
              text={selectedSubRace.details}
              className="flex flex-col gap-y-2"
            />
          </>
        )}
        {selectedClass?.details.map((detail: Detail) => (
          <div key={`class-${detail.title}`}>
            <p className="font-semibold mb-4">{detail.title}</p>
            <Html text={detail.description} className="flex flex-col gap-y-2" />
          </div>
        ))}
      </div>
    </div>
